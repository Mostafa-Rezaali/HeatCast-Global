#!/usr/bin/env python3
"""
Export frozen supervised spatial feature maps for all valid initialization times.

Run:
    python3 -u export_supervised_spatial_maps.py --device cuda --batch_size 8
"""
...
from met_jepa import (
    ...
    SUPERVISED_MAPS_PATH,
    SUPERVISED_META_PATH,
    MetJEPADataset,
...
def parse_args():
    parser = argparse.ArgumentParser(description="Export supervised spatial maps")
    parser.add_argument("--checkpoint", type=str, default=SUPERVISED_CHECKPOINT_PATH)
    parser.add_argument("--maps_path", type=str, default=SUPERVISED_MAPS_PATH)
    parser.add_argument("--meta_path", type=str, default=SUPERVISED_META_PATH)
...
def main():
    ...
    print(f"Loading supervised spatial encoder checkpoint: {args.checkpoint}")
    model, ckpt = load_supervised_checkpoint(args.checkpoint, device)
    stats = ckpt["stats"]
    ...
    loader = DataLoader(ds, batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers, pin_memory=torch.cuda.is_available())

    out_dtype = np.float16 if args.dtype == "float16" else np.float32
    print(f"Writing supervised spatial maps to {args.maps_path}")
    ...
    per_batch_vars = []
    exported = 0
    for context, global_fields, t in tqdm(loader, desc="Exporting supervised maps", mininterval=10.0):
        context = context.to(device, non_blocking=True)
        global_fields = global_fields.to(device, non_blocking=True)
        z = model.encode_maps(context, global_fields).float()
        if not torch.isfinite(z).all():
            raise ValueError("Non-finite values detected in exported supervised maps.")
    ...
    np.savez(
        ...
        supervised_maps_path=args.maps_path,
        ...
        artifact_type=np.array("supervised_spatial_feature_maps"),
        checkpoint_path=args.checkpoint,
    )
    print(f"Saved supervised-map metadata to {args.meta_path}")
