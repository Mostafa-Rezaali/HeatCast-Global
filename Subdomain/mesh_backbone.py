...
CHANGES:
  ...
  - GlobalContextForMesh: circular longitude padding/sampling for global fields
...
class PeriodicLonConv2d(nn.Conv2d):
    """Conv2d with circular longitude padding and non-periodic latitude padding.

    ERA5 global fields are periodic in longitude but not latitude. This wrapper
    keeps the same parameter names as nn.Conv2d while avoiding a zero-padding
    seam at the global field's left/right edge.
    """
...
class GlobalContextForMesh(nn.Module):
    ...
    def forward(self, global_fields, time_emb, mesh_lat, mesh_lon):
        ...
        # Append lon=360 as a duplicate of lon=0 so bilinear sampling is also
        # periodic at the seam. The conv blocks above already use circular lon
        # padding; this handles the final mesh-node interpolation step.
